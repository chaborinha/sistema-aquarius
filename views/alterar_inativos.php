...
$sql_alunos = "SELECT nome, idade, data_de_nascimento, email, altura, ativo, peso, remedio FROM alunos WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $ativo = (int) $ativo_post;

	// preparando a querry
	$sql = ("UPDATE alunos SET nome = ?, idade = ?, data_de_nascimento = ?,  email = ?,  altura = ?, ativo = ?, peso = ?, remedio = ? WHERE id = ?");
    ...
    $stmt->bindParam(6, $ativo);
    $stmt->bindParam(7, $peso);
    $stmt->bindParam(8, $remedio);
    $stmt->bindParam(9, $id_aluno);
